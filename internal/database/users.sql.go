...
const grantChirpyRed = `-- name: GrantChirpyRed :exec
UPDATE users
...
func (q *Queries) GrantChirpyRed(ctx context.Context, id uuid.UUID) error {
	_, err := q.db.ExecContext(ctx, grantChirpyRed, id)
	return err
}
